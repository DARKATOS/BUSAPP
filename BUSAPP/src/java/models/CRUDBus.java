...
public class CRUDBus {
    ...
    /**
     * bus Register: Metodo que permite el registro de un bus desde al
     * aplicación de servidor.
     *
     * @param plate Es la placa del bus.
     * @param password Contraseña con la que el conductor del bus puede acceder
     * a la aplicacion BUSAPP
     * @param driverName Nombre del conductor con el que esta relacionado el
     * bus.
     * @param busType Tipo de bus (básico, ejecutivo o especial).
     * @param ticketPrice Precio del pasaje del bus.
     * @return True si se realizo correctamente el registro. False si hubo un
    ...
    public boolean busRegister(String plate, String password, String driverName, String busType, int ticketPrice) {
        ...
        try {
            if (!"".equals(bus.getPlate()) && !"".equals(bus.getPassword()) && !"".equals(bus.getDriverName()) && !"".equals(bus.getBusType()) && bus.getTicketPrice() != 0) {
    ...
    /**
     * bus Show: Permite la visualización de los buses registrados en la
     * aplicación del servidor.
     *
     * @return ArrayList<Bus> retorna un lista de buses si se hizo la consulta
     * correctamente. o null si hubo un error.
     */
    ...
    /**
     * bus password: Metodo que permite obtener el password de alguna de las
     * cuentas de los buses.
     *
     * @param idbus Es la identificación del bus dentro de la base de datos del
     * servidor.
    ...
    /**
     * bus update: Metodo que permite la actualización de todos los datos de un
     * bus.
     *
     * @param id identificador del bus.
     * @param password Contraseña de la cuenta del bus.
     * @param passwordRepeat Repetición de la contraseña que debe coincidir con
     * el parametro password.
     * @param driverName Nombre del conductor del bus.
     * @param busType Tipo de bus (básico, ejecutivo o especial).
     * @param ticketPrice Precio del pasaje del bus.
     * @return True si se realizo correctamente la actualización de datos. False
     * si hubo un error en la actualización de datos.
     */
    public boolean busUpdate(int id, String password, String passwordRepeat, String driverName, String busType, int ticketPrice) {
        try {
            Bus bus = new Bus(id, null, password, driverName, busType, ticketPrice);
            EnableConnection connection = new EnableConnection();
            connection.setConnection();

            if (password.equals(passwordRepeat) && !"".equals(bus.getPassword()) && !"".equals(bus.getDriverName()) && !"".equals(bus.getBusType()) && bus.getTicketPrice() != 0) {
                CallableStatement execute = connection.executeCall("call bus_update(?,?,?,?,?)");
     ...
     * primera ubicación del bus, Con esto se considera activo dentro del
     * sistema BUSAPP, Este servicio es consumido desde la aplicacion
     * BusesBUSAPP
     *
     * @param idbus identificador del bus en el sistema.
     * @return String informando si se realizo correctamente el registro de la
     * ubicación del bus.
     */
    ...
    /**
     * bus location update service: Servicio que permite la actualización de la
     * ubicación del bus segun su recorrido, este servicio es consumido desde
     * laaplicación BusesBUSAPP.
     *
     * @param idbus Identificador del bus en el sistema.
     * @param latitude Nueva latitud que tiene el bus.
     * @param longitude Nueva longitud que tiene el bus.
     * @return String si se realizo correctamente la actualización de la
     * ubicación del bus.
     */
    ...
    /**
     * bus location delete service: Servicio que permite eliminar la ubicación
     * de un bus, se consume el servicio cuando el usuario se ha salido de la
     * aplicación BusesBUSAPP.
     *
     * @param idbus Identificador del bus en el sistema.
     * @return String indicando si se realizo correctamente el borrado de la
     * ubicacion del bus.
     */
    ...
    /**
     * bus login register service: Servicio que permite el inicio de sesión de
     * un bus y retorna los datos para guardar la sesión en un archivo de
     * configuración en la aplicación BusesBUSAPP, este servicio es consumido
     * desde la aplicación BusesBUSAPP.
     *
     * @param plate Placa del bus.
     * @param password Contraseña de la cuenta del bus.
     * @return Bus con los datos del bus que ha iniciado sesión. null si no se
     * encontro el bus en el sistema.
     */
    ...
    /**
     * bus login service; Servicio que permite el inicio de sesión de un bus a
     * traves de los datos almacenados en un archivo de configuración de la
     * aplicación BusesBUSAPP, el servicio es consumido desde la aplicación
     * BusesBUSAPP.
     *
     * @param idbus Identificador del bus en el sistema.
     * @param plate Placa del bus.
